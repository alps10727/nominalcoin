
import { Card, CardHeader, CardTitle } from "@/components/ui/card";
import { Link } from "react-router-dom";
import { LucideIcon } from "lucide-react";
import { ChevronRight } from "lucide-react";
import { useIsMobile } from "@/hooks/use-mobile";

interface MenuCardProps {
  title: string;
  icon: LucideIcon;
  to: string;
}

const MenuCard = ({ title, icon: Icon, to }: MenuCardProps) => {
  const isMobile = useIsMobile();
  
  return (
    <Link to={to}>
      <Card className="border-none shadow-sm hover:shadow-md transition-all duration-300 cursor-pointer group relative overflow-hidden">
        {/* Background gradient */}
        <div className="absolute inset-0 bg-gradient-to-br from-gray-900/95 via-gray-850/95 to-indigo-950/95 opacity-90 group-hover:opacity-100 transition-opacity"></div>
        
        {/* Hover effect - left border highlight */}
        <div className="absolute left-0 top-0 bottom-0 w-1 bg-violet-600 scale-y-0 group-hover:scale-y-100 transition-transform origin-bottom duration-300"></div>
        
        {/* Pattern overlay */}
        <div className="absolute inset-0 bg-[url('data:image/svg+xml;base64,')] bg-fixed opacity-[0.05] group-hover:opacity-[0.08] transition-opacity duration-500 backdrop-blur-sm"></div>
        
        <CardHeader className={`${isMobile ? 'p-3' : 'p-4'} relative z-10`}>
          <CardTitle className={`${isMobile ? 'text-sm' : 'text-md'} flex justify-between items-center text-gray-100`}>
            <div className="flex items-center">
              <div className={`${isMobile ? 'p-1.5' : 'p-2'} rounded-lg bg-gradient-to-br from-violet-800/50 to-indigo-900/50 group-hover:from-violet-700/60 group-hover:to-indigo-800/60 transition-colors mr-2 border border-violet-500/20 backdrop-blur-sm`}>
                <Icon className={`${isMobile ? 'h-4 w-4' : 'h-5 w-5'} text-violet-300 group-hover:text-violet-200 transition-colors`} />
              </div>
              <span className="group-hover:translate-x-0.5 transition-transform duration-300">{title}</span>
            </div>
            <ChevronRight className={`${isMobile ? 'h-4 w-4' : 'h-5 w-5'} text-violet-500/70 group-hover:text-violet-400 group-hover:translate-x-0.5 transition-all duration-300`} />
          </CardTitle>
        </CardHeader>
      </Card>
    </Link>
  );
};

export default MenuCard;
