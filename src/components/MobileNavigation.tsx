...
const MobileNavigation = () => {
  ...
  return (
    <nav className="bg-gradient-to-r from-gray-900 via-gray-850 to-gray-900 backdrop-blur-xl border-t border-teal-500/20 fixed bottom-0 left-0 right-0 flex justify-around p-3 z-10 shadow-[0_-4px_20px_rgba(0,0,0,0.4)] pb-safe">
      <NavItem 
        to="/" 
        icon={Zap} 
        label={t('nav.mining')} 
  ...
  return (
    <Link to={to} className="flex flex-col items-center transition-all duration-300 relative z-10">
      <div className={`p-2 rounded-xl transition-all duration-300 ${isActive 
        ? 'bg-gradient-to-r from-teal-600 to-emerald-600 text-white shadow-lg shadow-teal-900/40 scale-110' 
        : 'text-gray-400 hover:bg-gray-800/60 hover:text-teal-300'}`}>
