...
const MobileNavigation = () => {
  ...
  return (
    <nav className="bg-gray-900/90 backdrop-blur-md border-t border-gray-800 fixed bottom-0 left-0 right-0 flex justify-around p-3 z-10 shadow-[0_-1px_5px_rgba(0,0,0,0.3)]">
      ...
      <Link to="/tasks" className="flex flex-col items-center text-gray-500">
        <div className={`p-1.5 rounded-full ${location.pathname === '/tasks' ? 'bg-indigo-900/50 text-indigo-400' : ''}`}>
          <Award className="h-5 w-5" />
        </div>
        <span className={`text-xs mt-1 ${location.pathname === '/tasks' ? 'font-medium text-indigo-400' : ''}`}>{t('nav.tasks')}</span>
      </Link>
